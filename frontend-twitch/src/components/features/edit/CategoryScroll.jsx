...
const CategoryScroll = ({ key }) => {
    ...
    const categories = [
        ...
        {id: 3, name: "MineCraft", thumbnail: "/images/categories/game3.jpg", interested: "236000"},
    ...
    useEffect(() => {
        setSearchCategories(categories.filter(
            each => each.name.toLowerCase().includes(key.toLowerCase())
        ));
    }, [key])
    ...
    return (
        <CategoryScrollContainer>
            {searchCategories.map(category => <div>{category.name}</div>)}
        </CategoryScrollContainer>
    );
};
